...
export function WhyUs() {
  return (
    ...
    <section id="dlaczego-my" className="scroll-mt-40 bg-mint py-10 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <p className="flex items-center justify-center gap-2 text-xs sm:text-sm font-semibold uppercase tracking-[0.16em] text-navy">
            <Award className="size-4 text-green" aria-hidden="true" />
            Dlaczego My
          </p>
        ...
        <ul className="mt-6 grid grid-cols-2 gap-3 sm:mt-12 sm:gap-6 lg:grid-cols-4">
          {reasons.map((item) => {
            const Icon = item.icon;
            return (
              <li
                key={item.title}
                className="flex flex-col items-center rounded-2xl border border-black/5 bg-white p-4 text-center shadow-sm transition-all hover:border-green/50 hover:shadow-md sm:rounded-3xl sm:p-6"
              >
                <span className="flex size-10 items-center justify-center rounded-full bg-navy text-green shadow-md sm:size-14">
                  <Icon className="size-5 sm:size-6" aria-hidden="true" />
                </span>
                <h3 className="mt-3 font-display text-[13px] font-bold uppercase tracking-wide text-navy sm:mt-5 sm:text-lg sm:tracking-wider">
                  {item.title}
                </h3>
                <p className="mt-1.5 text-xs leading-relaxed text-ink/75 sm:mt-3 sm:text-sm">
                  {item.text}
                </p>
              </li>
            );
        ...
        <div className="mt-8 rounded-2xl border border-black/10 bg-mint-dark p-5 shadow-sm sm:mt-16 sm:rounded-3xl sm:p-8 lg:p-12">
          <div className="grid items-center gap-6 lg:grid-cols-12 lg:gap-12">
            <div className="text-center sm:text-left lg:col-span-5">
              <p className="flex items-center justify-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-navy sm:justify-start">
                <Handshake className="size-4 text-green" aria-hidden="true" />
                Zaufana współpraca
              </p>
              ...
              <div className="mt-4 sm:mt-6">
                ...
                >
                  {/* Bez style={{height:"auto"}} – styl wpisany bezpośrednio
                      wygrywał z klasami h-8/sm:h-10 i unieważniał skalowanie. */}
                  <Image
            ...
            <ul className="grid grid-cols-1 gap-2.5 sm:grid-cols-3 sm:gap-4 lg:col-span-7">
              {partnerFeatures.map((f) => {
                const Icon = f.icon;
                return (
                  <li
                    key={f.title}
                    className="flex items-center gap-3 rounded-2xl border border-black/5 bg-white p-3.5 shadow-sm transition-all sm:flex-col sm:items-start sm:gap-0 sm:p-5"
                  >
                    <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-navy/5 text-navy sm:size-9 sm:rounded-xl">
                      <Icon className="size-4 sm:size-5" aria-hidden="true" />
                    </span>
                    <div className="min-w-0">
                      <h4 className="font-display text-sm font-bold text-ink sm:mt-3">
                        {f.title}
                      </h4>
                      <p className="text-xs leading-relaxed text-ink/70 sm:mt-1.5">
                        {f.text}
                      </p>
                    </div>
                  </li>
