...
// Elementy, na które można przejść klawiszem Tab wewnątrz okna ustawień.
...
export function CookieBanner() {
  ...
  if (isSettingsOpen) return <CookieSettingsDialog />;
        ...
        <div className="flex flex-col gap-5 p-5 sm:p-7 lg:flex-row lg:items-center lg:justify-between lg:gap-8">
          <div className="flex items-start gap-4 lg:items-center">
            ...
            <div className="min-w-0">
              {/* Zwykły akapit zamiast nagłówka – baner nie jest częścią
                  treści strony i nie powinien zaburzać struktury nagłówków. */}
              ...
              <p className="mt-1.5 max-w-2xl text-xs leading-relaxed text-white/70 sm:text-sm">
                Cookies niezbędne stosujemy zawsze. Statystyki Google Analytics
                uruchamiamy wyłącznie po Twojej akceptacji.{" "}
                <Link
          ...
          <div className="flex shrink-0 flex-col gap-2.5 sm:flex-row sm:items-center lg:justify-end">
            ...
            >
              <Settings2 className="size-4 text-green" aria-hidden="true" />
              Ustawienia
            </button>
            <button
              type="button"
...
function CookieSettingsDialog() {
  const { consent, acceptAll, savePreferences, closeSettings } =
    useCookieConsent();

  // Okno montuje się od nowa przy każdym otwarciu, więc przełącznik zawsze
  // startuje od aktualnie zapisanej zgody (domyślnie wyłączony).
  ...
  useEffect(() => {
    ...
    // 1. Zapamiętaj, co miało fokus, i przenieś fokus do okna.
    const previouslyFocused = document.activeElement as HTMLElement | null;
    modal.focus();

    // 2. Zablokuj przewijanie strony pod oknem. Szerokość paska przewijania
    //    dodajemy jako margines, żeby strona nie "przeskakiwała" w bok.
    ...
    // 3. Escape zamyka okno, a Tab krąży tylko po elementach okna.
    ...
    // 4. Po zamknięciu wszystko wraca do stanu sprzed otwarcia.
    return () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-navy-900/60 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      <div
        ref={modalRef}
        role="dialog"
        aria-modal="true"
        ...
        tabIndex={-1}
        className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-white p-6 text-ink shadow-2xl outline-none sm:rounded-3xl sm:p-8"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-navy">
              <Cookie className="size-4 text-green" aria-hidden="true" />
              Ustawienia cookies
            </p>
            <h2
        ...
        <div className="mt-6 space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-ink/65">
                  Wymagane do działania podstawowych funkcji serwisu. Nie można
                  ich wyłączyć.
                </p>
              </div>
            </div>
          ...
          <div className="flex items-start justify-between gap-4 rounded-2xl border border-black/10 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p
                  ...
                  className="mt-1 text-xs leading-relaxed text-ink/65"
                >
                  Google Analytics – pomaga nam weryfikować liczbę odwiedzin i
                  optymalizować działanie strony.
                </p>
              </div>
            </div>
            <label className="relative mt-1 inline-flex shrink-0 cursor-pointer items-center">
              ...
              <span
                aria-hidden="true"
                className="block h-6 w-11 rounded-full bg-black/15 transition-colors after:absolute after:top-0.5 after:left-0.5 after:size-5 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:bg-navy peer-checked:after:translate-x-5 peer-focus-visible:ring-2 peer-focus-visible:ring-green peer-focus-visible:ring-offset-2"
              />
            </label>
          </div>
        </div>
